...
{
	PeoInfo tmp = { 0 };
	FILE* pfRead = fopen("通讯录.text", "rb");
	if (pfRead == NULL)
	{
		printf("LoadContact::%s\n", strerror(errno));
		return;
	}
	while (fread(&tmp, sizeof(PeoInfo), 1, pfRead))//从文件内读取全部信息放入通讯录中
	{
		Check_Capacity(ps);
		ps->data[ps->size]=tmp;
		ps->size++;
...
{
	ps->data = (PeoInfo*)malloc(Default_size * sizeof(PeoInfo));//通讯录初始化（动态分配）能存放3个好友信息的内存并且加载通讯录文件中的信息
	if (ps->data == NULL)
	{
...
void Check_Capacity(Contact* ps)//检测容量函数（满了，增容；没满,啥事不干）
{                                            
...
void SaveContact(const Contact* ps)//保存通讯录信息函数（保存到文件）
{
	FILE* pfWrite = fopen("通讯录.text","wb");
	if (pfWrite == NULL)
	{
		fprintf("SaveContact::%s\n", strerror(errno));//文件打开失败，打印失败原因后，立即返回
		return;
	}
	int i = 0;
	for (i = 0; i < ps->size; i++)//从通讯录中一个信息一个信息的写入文件内
	{
		fwrite(&(ps->data[i]), sizeof(PeoInfo), ps->size, pfWrite);
	}
	fclose(pfWrite);//关闭文件
	pfWrite = NULL;
}
